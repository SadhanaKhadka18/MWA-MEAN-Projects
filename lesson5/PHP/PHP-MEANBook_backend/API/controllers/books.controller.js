...
const _runGeoQuery = function (req, res, offset, count) {
    ...
    Book.find(query).skip(offset).limit(count).exec().then(function (books) {
        if (!books) {
...
const getAll = function (req, res) {
    ...
    if (count > maxCount) {
        ...
        Book.find().skip(offset).limit(count).exec().then(function (books) {
            if (!books) {
                console.log("No Books added");
...
const addOne = function (req, res) {
    ...
    Book.create(newBook).then(function (addedBook) {
...
const deleteOne = function (req, res) {
    ...
    Book.findByIdAndDelete(bookId).exec().then(function (deletedBook) {
        if (!deletedBook) {
            _setResponse(response, process.env.NOT_FOUND_STATUS_CODE, { "message": process.env.BOOK_NOT_FOUND_MESSAGE });
        } else {
